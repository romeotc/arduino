...
#define server "blynk.honey.co.th"   // or "blynk.cloud-com" for Blynk's cloud server
#define TestLED 2                 // on board LED pin assignment
char Date[16];
char Time[16];
char auth[] = "AbNud7e0K9f6pfXrmAXnxM0qTKJXqBGQ";
...
if (isFirstConnect) {
  Blynk.syncAll();
  Blynk.notify("ระบบตั้งเวลาทำงาน!!!!");
...
BLYNK_WRITE(V1)  // Manual/Auto selection
{
  if (param.asInt()==1) {
    ...
    terminal.println();
    terminal.println();
    terminal.println("เปิด Manual MODE");
    terminal.println("กดปุ่ม ON/OFF button ถ้าต้องการสั่งงาน");
    terminal.println();
    terminal.println();
    terminal.println();
   // terminal.flush();
    ...
    terminal.println();
    terminal.println();
    terminal.println("ปิด Manual MODE");
